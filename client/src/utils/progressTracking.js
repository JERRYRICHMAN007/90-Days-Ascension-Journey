  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent('session-completed', {
        detail: { journeyId, dayNumber, sessionType, sessionIndex, discipline },
      })
    );
...
/**
 * Collect all session keys defined for a day in the journey structure
 */
export function collectDaySessionKeys(journeyId, day) {
  const keys = [];

  if (day.schedule?.scheduledContent?.deepLearning) {
    day.schedule.scheduledContent.deepLearning.forEach((session, idx) => {
      keys.push(getSessionKey(journeyId, day.dayNumber, 'deepLearning', idx, session.discipline || null));
    });
  }

  if (day.schedule?.scheduledContent?.focusedImplementation) {
    day.schedule.scheduledContent.focusedImplementation.forEach((session, idx) => {
      keys.push(getSessionKey(journeyId, day.dayNumber, 'focusedImplementation', idx, session.discipline || null));
    });
  }

  if (!day.schedule?.scheduledContent && (day.workout || day.readingSessions || day.execution || day.personalBrandTasks)) {
    keys.push(getSessionKey(journeyId, day.dayNumber, 'daily', 0));
  }
...
/**
 * Mark or clear all sessions for a day (used by Mark Complete)
 */
export function setDaySessionsComplete(journeyId, day, complete) {
  if (!day || day.dayNumber === 0) return false;
  ...
  try {
    ...
    keys.forEach((key) => {
      if (complete) {
        completions[key] = { completed: true, completedAt: new Date().toISOString() };
      } else {
    ...
    localStorage.setItem('sessionCompletions', JSON.stringify(completions));
    window.dispatchEvent(
      new CustomEvent('session-completed', { detail: { journeyId, dayNumber: day.dayNumber } })
    );
    window.dispatchEvent(new CustomEvent('progress-updated', { detail: { journeyId, dayNumber: day.dayNumber } }));
    return true;
  } catch (error) {
    console.error('Error setting day sessions complete:', error);
    return false;
  }
}

/**
 * Sync backend completed day numbers into sessionCompletions
 */
export function mergeBackendCompletedDays(journeyId, completedDayNumbers, weeks) {
  if (!weeks || !Array.isArray(completedDayNumbers)) return;

  const allDays = weeks.flatMap((w) => w.days || []).filter((d) => d && d.dayNumber > 0);

  completedDayNumbers.forEach((dayNumber) => {
    const day = allDays.find((d) => d.dayNumber === dayNumber);
    if (day) {
      setDaySessionsComplete(journeyId, day, true);
    }
  });
}

/**
 * Sync a day completion to the backend API
 */
export async function syncDayCompletionToBackend(journeyId, dayNumber, completed, sessionKey = null) {
  try {
    const accessToken = localStorage.getItem('accessToken');
    if (!accessToken) return;

    const { api } = await import('../services/api');
    await api.completeTask(journeyId, dayNumber, completed, sessionKey);
  } catch (error) {
...
/**
 * Toggle day complete state and optionally sync to backend
 */
export async function toggleDayComplete(journeyId, day, complete) {
  setDaySessionsComplete(journeyId, day, complete);
  await syncDayCompletionToBackend(journeyId, day.dayNumber, complete);
}
 ...
 * Use this when starting fresh on Day 0 (June 30, 2026)

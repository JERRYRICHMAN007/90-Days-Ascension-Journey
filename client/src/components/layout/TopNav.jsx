...
import { Moon, Sun, Sparkles, Search, Bell, Flame, Menu } from 'lucide-react';
...
export function TopNav() {
  ...
  const syncDotColor = isOnline ? '#00ff87' : '#555555';
      ...
      <div className="flex items-center justify-between h-14 px-3 sm:px-4 md:px-6 gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
          ...
          >
            ...
            <span className="text-sm md:text-base font-bold text-white hidden sm:inline" style={{ letterSpacing: '0.05em' }}>
              Forge90
            </span>
            <span
              className="w-2 h-2 rounded-full shrink-0 hidden sm:inline-block"
              style={{ backgroundColor: syncDotColor, boxShadow: isOnline ? 'var(--neon-glow-green)' : 'none' }}
              title={isOnline ? 'Synced' : 'Offline'}
              aria-label={isOnline ? 'Synced' : 'Offline'}
            />
          </button>
          ...
          {currentPhaseStatus === 'before' && (
            <div className="text-xs text-muted-foreground truncate hidden sm:inline px-2 py-0.5 rounded bg-muted/30">
              Day 0: Jun 30 • Starts Jul 1, 2026
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          ...
          <Button 
            variant="ghost" 
            size="sm" 
            className="relative h-11 w-11 p-0 touch-manipulation hover:bg-muted/50 shrink-0"
          ...
          <Button
            variant="ghost"
            size="icon"
            className="sm:hidden h-11 w-11 touch-manipulation shrink-0"
            onClick={toggleTheme}
          ...
          <button
            ...
            className="relative h-11 w-11 rounded-full overflow-hidden border-2 border-border hover:border-primary transition-colors touch-manipulation shrink-0"
            aria-label="Profile"
          >
            {user?.avatarUrl ? (
              <img
